...
import vandy.mooc.provider.AcronymProvider;
...
public class ContentProviderTimeoutCache implements
	...
	public List<AcronymExpansion> get(final String acronym) {
		...
		try (Cursor cursor = mContext.getContentResolver().query(
			...
			// from first row and add it to the List.
				...
				Log.v(AcronymProvider.TAG, "Cursor not null and has first item");
				Long expirationTime = cursor
						.getLong(cursor
								.getColumnIndex(AcronymContract.AcronymEntry.COLUMN_EXPIRATION_TIME));
	...
	private int putValues(String acronym, List<AcronymExpansion> longForms,
		...
		for (int i = 0; i < longForms.size(); i++) {
			...
			AcronymExpansion currAcronym = longForms.get(i);

			cvs.put(AcronymEntry.COLUMN_ACRONYM, acronym);
			cvs.put(AcronymEntry.COLUMN_EXPIRATION_TIME, expirationTime);
			cvs.put(AcronymEntry.COLUMN_LONG_FORM, currAcronym.getLf());
			cvs.put(AcronymEntry.COLUMN_FREQUENCY, currAcronym.getFreq());
			cvs.put(AcronymEntry.COLUMN_SINCE, currAcronym.getSince());
